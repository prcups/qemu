...
static inline CPUSysState *get_sys_state(CPULoongArchState *env, int vm_level)
{
...
{
    return get_sys_state(env, guest ? LOONGARCH_VM_LEVEL_GUEST :
                                      LOONGARCH_VM_LEVEL_HOST);
}

static inline int env_vm_level(CPULoongArchState *env)
{
    return env_sys(env) == get_sys_state(env, LOONGARCH_VM_LEVEL_GUEST) ?
